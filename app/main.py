...
class GrammarGameApp(ttk.Frame):
    ...
    def __init__(self, master: tk.Tk, bank: QuestionBank, store: ProgressStore) -> None:
        ...
        self._mistakes: dict[str, Question] = {}
        self._review_queue: list[Question] = []
    ...
    def _build_layout(self) -> None:
        ...
        submit_btn = ttk.Button(actions, text="Check Answer", command=self._check_answer, style="Accent.TButton")
        ...
        audio_btn.grid(row=0, column=4, padx=(8, 8))

        review_btn = ttk.Button(actions, text="Review Mistakes", command=self._start_review)
        review_btn.grid(row=0, column=5)
    ...
    def _render_question(self) -> None:
        ...
        if self._review_mode:
            self._group_label.set(f"Review Mode | Mistakes left: {len(self._review_queue)}")
        else:
    ...
    def _check_answer(self) -> None:
        ...
        if is_correct:
            ...
            self._mistakes[self._current.qid] = self._current
    ...
    def _next_question(self) -> None:
        ...
        if self._review_mode:
            if not self._review_queue:
                self._review_mode = False
                self._feedback_text.set("Review complete. Returning to practice mode.")
                self._current = self._bank.next_question()
            else:
                self._current = self._review_queue.pop()
        else:
            self._current = self._bank.next_question()
        self._render_question()
    ...
    def _next_group(self) -> None:
        if self._review_mode:
            self._feedback_text.set("Finish review mode before changing groups.")
            return
    ...
    def _start_review(self) -> None:
        if not self._mistakes:
            self._feedback_text.set("No mistakes saved yet. Keep practicing!")
            return
        self._review_queue = list(self._mistakes.values())
...
def main() -> None:
    ...
    style.configure("TLabel", background="#f4f6fb", foreground="#1f2933")
    style.configure("TLabelframe", background="#f4f6fb", foreground="#1f2933")
    style.configure("TLabelframe.Label", background="#f4f6fb", foreground="#3a506b")
    style.configure("TButton", padding=6)
    style.configure("Accent.TButton", background="#3d5afe", foreground="#ffffff")
    style.map("Accent.TButton", background=[("active", "#2f48d6")])
